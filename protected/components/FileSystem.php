...
class FileSystem extends CComponent
{
    ...
    public function getCropedImageUrl($uid, $size) {
        $fileName = pathinfo($uid, PATHINFO_FILENAME);
        $ext = pathinfo($uid, PATHINFO_EXTENSION);
        if (!empty($ext))
            $ext = '.'.$ext;
    ...
    public function cropImage($uid, $sizes = array(), $forceCreate = false, $resizeFirst = false) {
        /** @var $cImage CImageComponent */
        $cImage = Yii::app()->image;
        $imageFile = $this->getFilePath($uid);
        $pathInfo = pathinfo($imageFile);
        ...
        foreach($sizes as $size) {
            ...
            if($resizeFirst)
            {
                if($originalImage->width>$originalImage->height)
                    $proc = $originalImage->height/$size[0];
                else
                    $proc = $originalImage->width/$size[0];

                $image
                        ->crop($size[0]*$proc, $size[1]*$proc, 'center', 'center')
                        ->resize($size[0], $size[0], Image::AUTO)
                        ->quality($this->jpegQuality);
            }
            else
            {
                $image->crop($size[0], $size[1], 'center', 'center')->quality($this->jpegQuality);
            }
